...
import { initSettings, loadAvatar, loadPrefsFromServer, applyThemeSlots, activateTheme, deactivateTheme, openThemeSelector, openThemeEditor, loadPresets, refreshPresets } from './settings.js';
...
window.__theme = { activateTheme, deactivateTheme, applyThemeSlots, openThemeSelector, openThemeEditor, loadPresets, refreshPresets };
...
async function init() {
  ...
  loadPrefsFromServer();
  loadLikes();
  import('./ui.js').then(({ loadFavMids }) => loadFavMids().then(() => import('./ui.js').then(({ updateFavsCount }) => updateFavsCount())));
