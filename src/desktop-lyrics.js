...
html,body{width:100%;height:100%;overflow:hidden;font-size:14px;background:#15171a}
.dt-root{
...
export async function openDesktopLyrics() {
  ...
  if ('documentPictureInPicture' in window) {
    try {
      ...
      pipWindow.document.head.innerHTML = `<meta name="theme-color" content="#15171a"><style>${PIP_CSS}</style>`;
      pipWindow.document.body.innerHTML = buildHTML(state.current);
      setThemeColor(pipWindow.document, pipWindow.document.querySelector('.dt-root.dt-light') ? 'light' : 'dark');
  ...
  doc.querySelectorAll('.dtopt').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (s === 'mode') {
        ...
        if (v === 'light') { root.classList.add('dt-light'); setThemeColor(doc, 'light'); }
        else if (v === 'dark') { root.classList.remove('dt-light'); setThemeColor(doc, 'dark'); }
        else if (v === 'system') {
          const prefersDark = doc.defaultView.matchMedia('(prefers-color-scheme: dark)').matches;
          root.classList.toggle('dt-light', !prefersDark);
          setThemeColor(doc, prefersDark ? 'dark' : 'light');
        }
      }
...
// 更新 theme-color meta，使顶栏与内容区背景融为一体
function setThemeColor(doc, mode) {
  const meta = doc.querySelector('meta[name="theme-color"]');
  if (meta) meta.setAttribute('content', mode === 'light' ? '#f0f2f5' : '#15171a');
}
...
function openPopup() {
  ...
  pop.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="theme-color" content="#15171a"><style>${PIP_CSS}</style></head><body>${buildHTML(state.current)}</body></html>`);
  pop.document.close();
  pipWindow = pop;
