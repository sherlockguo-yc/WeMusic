...
**Architecture:** 纯前端实现（无新增服务端接口）。新建 `src/offlineCache.js` 作为缓存存储与逻辑核心，底层用 IndexedDB 存 `{audio 字节, videoSource, lyrics, pinned, lastAccessed, size}`。播放路径（`player.js` 的 `_getPlayableUrl`）先查缓存命中；未命中则走现有直连/代理流，同时后台经**同源代理 `/api/play/stream`** 抓取完整字节落盘（绕开 B站 CDN 的跨域 fetch 限制）。右键菜单（`ui.js`）承载主动钉住，侧边栏独立页面（`offline-page.js`）承载管理 UI。
...
## Task 5: 侧边栏「离线缓存」独立页面（替代原设置页方案）

> **2026-07-17 更新**：缓存管理已从设置面板移出，改为侧边栏独立页面（`src/offline-page.js`），入口为侧边栏「离线缓存」导航项（位于「我的专辑」下方）。

**Files:**
- Create: `src/offline-page.js`（独立页面模块，含 `openOfflinePage()` + 渲染/事件绑定）
- Modify: `public/index.html`（侧边栏新增 `navOffline` 导航项；移除 `#settingsModal` 内 `offlineSection`）
- Modify: `src/stats.js`（`initStats()` 绑定 `navOffline` 点击）
- Modify: `src/playlist-ui.js`（`setActiveNav` 加入 `navOffline`）
- Modify: `src/main.js`（`restoreView` 添加 `offline` 路由恢复）
- Modify: `src/settings.js`（移除离线缓存 section 全部逻辑：导入、渲染、事件绑定）
- Modify: `public/css/style.css`（新增 `.offline-page-bar` / `.offline-page-list` 样式）
- Test: `tests/unit/offlineCache.test.js` 已覆盖核心逻辑

**页面结构：**
```
.view-title         → 离线缓存
.view-sub           → X 首歌曲 · X 首主动缓存 · X 首自动缓存 · 已用 X.XX GB / X GB
.offline-page-bar   → 存储上限选择器 + [清空自动缓存] [清空全部]
.section-head h2    → 已缓存歌曲
.offline-page-list  → 缓存条目列表（.offline-item 复用已有样式）
```

**与 Task 5 原方案的关键差异：**
- 不再依赖 `#settingsModal`，改为独立视图（`main.innerHTML` 渲染）
- 不再折叠列表，全部展开显示
- 导航激活状态通过 `setActiveNav('navOffline')` 管理
- 路由支持前进/后退（`navPush('offline')` + `restoreView` 恢复）
