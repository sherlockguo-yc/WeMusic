...
- **管理员面板**：顶栏盾牌图标入口，查看用户列表（含最近登录时间）+ 用户详情（播放次数/总时长/歌曲/歌单数）+ 管理反馈。
...
- 自动降权伴奏/现场/翻唱版本（BAD_KW 扣分 + 现场版置底），候选标记当前源。
...
### 界面
- 深色/浅色/跟随系统，20 色预设主题 + 自定义主题色（拾色器，最多 8 个，跨设备同步），字号 4 档，字体 9 种。
...
- iframe 模式下进度靠计时器估算（跨域无法读取 B 站播放器真实进度），展开视频时自动切歌可能有偏差。后台 bg 模式通过 `<audio>` 的 `ended` 事件切歌，准确无偏差。
- iframe 模式下进度/画质不可控（跨域限制），音量可通过 B 站 postMessage 协议控制。
